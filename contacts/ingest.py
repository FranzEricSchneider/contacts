...
def validate_similar_names(
    new_names: List[str], existing_names: List[str], text_file_path: Path
) -> List[str]:
...
def update_contacts(yaml_path: Path, text_file_path: Path) -> None:
    ...
    contacts = {contact.name: contact for contact in existing_contacts}
